...
# Android 10+: bionic linker and runtime live under /apex; required for guest ELF exec.
if [[ -d /apex ]] ; then
	EXTRA_BINDINGS="$EXTRA_BINDINGS -b /apex:/apex"
fi
if [[ ! -r /dev/urandom ]] ; then
	EXTRA_BINDINGS="$EXTRA_BINDINGS -b /dev/urandom:/dev/random"
fi
...
# Launch PRoot via the system linker (required on Android 10+ when binaries live in APK .so paths).
unset LD_PRELOAD
PROOT_LINKER=/system/bin/linker64
if [[ ! -x "$PROOT_LINKER" ]] ; then
	PROOT_LINKER=/system/bin/linker
fi
...
PROOT_NO_SECCOMP=1 \
$PROOT_LINKER "$PROOT" \
-r $ROOTFS_PATH -v $PROOT_DEBUG_LEVEL -p --sysvipc -H -0 -l -L \
-b /sys -b /dev -b /proc -b /data -b /mnt \
